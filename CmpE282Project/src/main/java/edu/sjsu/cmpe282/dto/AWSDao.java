...
public class AWSDao {
	...
	public void QueryProducts(String category) {
		 ...
		 * System
		 * .out.println("item1 price : "+result.get(0).get("price").getN());
		 * System
		 * .out.println("item1 image : "+result.get(0).get("image").getS());
		 * System
		 * .out.println("item1 category : "+result.get(0).get("category").getS
		 * ());
		 * System.out.println("item1 product_id : "+result.get(0).get("product_id"
		 * ).getN());
		 */
		List<Map<String, AttributeValue>> result = scanResult.getItems();
		for (int i = 0; i < result.size(); i++) {
			String name = result.get(i).get("product_name").getS();
			String price = result.get(i).get("price").getN();
			String url = result.get(i).get("image").getS();
			String cat = result.get(i).get("category").getS();
			String productid = result.get(i).get("product_id").getS();
	...
	// Method overloading...No argument method QueryProduct() to generate
	// home-page without specifying category
	public void QueryProducts() {
		System.out.println("Creating home page...");
		HashMap<String, Condition> scanFilter = new HashMap<String, Condition>();
		Condition condition = new Condition().withComparisonOperator(
				ComparisonOperator.EQ.toString()).withAttributeValueList(
				new AttributeValue().withS("Fashion"));
		scanFilter.put("category", condition);
		ScanRequest scanRequest = new ScanRequest(tableName)
				.withScanFilter(scanFilter);
		ScanResult scanResult = client.scan(scanRequest);
	...
	public String QueryHTMLProducts() {
		...
		Freemarker fm = new Freemarker();
		fm.setCategoryTemplate();
		return fm.writeHTMLOutput(products, "Home");
	}
	...
	public void describeProduct(String productid) {
		...
		List<Map<String, AttributeValue>> result = scanResult.getItems();
		productMap
				.put("product_name", result.get(0).get("product_name").getS());
		...
		productMap.put("product_id", result.get(0).get("product_id").getS());
